...
class KRadarDetection_v2_0(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # ---- temporal meta only (NO prev frame loading here) ----
        seq = dict_item['meta'].get('seq', None)
        
        curr_idx = None
        if 'idx' in dict_item['meta']:
            curr_idx = dict_item['meta']['idx'].get('rdr', None)
        
        # 强制保存一个 int 版本，供 TMC 使用
        try:
            dict_item['meta']['rdr_idx_int'] = int(curr_idx)
        except Exception:
