...
class BaseBEVBackbone_MF(nn.Module):
    def __init__(self, model_cfg, input_channels):
        ...
        # If enabled, compute a radar weight map w(x,y) in [0,1] conditioned on
        #   [F_lidar, F_radar, C_radar] where C_radar is a BEV confidence map.
        # Then reweight features before the MF backbone:
        #   F_l' = (1-w) * F_l,   F_r' = w * F_r
        uagf_cfg = self.model_cfg.get('UAGF', {}) if hasattr(self.model_cfg, 'get') else getattr(self.model_cfg, 'UAGF', {})
        self.uagf_enabled = bool(uagf_cfg.get('ENABLED', False))
        self.uagf_conf_key = str(uagf_cfg.get('CONF_KEY', 'radar_conf_bev'))
        ...
        if self.uagf_enabled:
            ...
            self.uagf_gate = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Conv2d(self.uagf_hidden, 1, kernel_size=1, bias=True),
                nn.Sigmoid()
            )
        else:
            self.uagf_gate = None
def forward(self, data_dict):
        ...
        ups = []
        ret_dict = {}
        # UAGF: reweight lidar/radar BEV features using radar confidence map (if provided)
        if self.uagf_enabled and (self.uagf_gate is not None):
            conf = data_dict.get(self.uagf_conf_key, None)
            if conf is not None:
                if conf.dim() == 3:
                    conf = conf.unsqueeze(1)
                # resize conf to match feature resolution if needed
                if conf.shape[-2:] != lidar_x.shape[-2:]:
                    conf = F.interpolate(conf, size=lidar_x.shape[-2:], mode='bilinear', align_corners=False)
                # clamp to [0,1] if caller provides unnormalized map
                conf = conf.clamp(0.0, 1.0)
                gate_in = torch.cat([lidar_x, radar_x, conf], dim=1)
            else:
                # fallback: gate conditioned only on features
                gate_in = torch.cat([lidar_x, radar_x, lidar_x.new_zeros((lidar_x.shape[0],1,*lidar_x.shape[2:]))], dim=1)
            w = self.uagf_gate(gate_in)  # (B,1,H,W)
            data_dict['uagf_w'] = w
            lidar_x = lidar_x * (1.0 - w)
            radar_x = radar_x * w

        # -------- periodic debug print --------
        if self.training:
            self._debug_iter += 1
            if self._debug_iter % self._debug_interval == 0:
                w_mean = data_dict['uagf_w'].mean().item()
                w_min  = data_dict['uagf_w'].min().item()
                w_max  = data_dict['uagf_w'].max().item()
                print(f"[UAGF][Iter {self._debug_iter}] mean={w_mean:.4f}, min={w_min:.4f}, max={w_max:.4f}")
