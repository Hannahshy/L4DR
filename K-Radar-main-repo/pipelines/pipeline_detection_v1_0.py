...
class PipelineDetection_v1_0():
    def __init__(self, path_cfg=None, mode='train', rank = 0, tag = 'default'):
        ...
        *   'test'  denotes mode for inference
        ...
        if self.dist:
            ...
            self.test_sampler = torch.utils.data.distributed.DistributedSampler(self.dataset_test)
        if self.rank == 0:
    ...
    def train_network(self, is_shuffle=True):
        ...
        self.network.train()
        if self.dist:
            self.network=torch.nn.parallel.DistributedDataParallel(self.network, device_ids=[self.rank % torch.cuda.device_count()])
            data_loader_train = torch.utils.data.DataLoader(self.dataset_train, \
                ...
                num_workers = self.cfg.OPTIMIZER.NUM_WORKERS, drop_last = True, sampler=self.train_sampler)
        else:
            data_loader_train = torch.utils.data.DataLoader(self.dataset_train, \
        ...
        for epoch in range(epoch_start, epoch_end):
            if self.dist:
                self.train_sampler.set_epoch(epoch + 10086)
            if self.rank==0:
